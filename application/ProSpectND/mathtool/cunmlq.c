...
#define C(I,J) c[(I)-1 + ((J)-1)* ( ldc)]
   ...
   Writing concatenation */
    ...
    i__1 = 64, i__2 = ilaenv(c__1, "CUNMLQ", ch__1, m__, n__, k__, c_n1, 6L, 2L);
    nb = min(i__1,i__2);
    nbmin = 2;
    ldwork = nw;
    if (nb > 1 && nb < k__) {
	...
	if (lwork < iws) {
	    ...
	    i__3[1] = 1, a__1[1] = trans;
	    s_cat(ch__1, a__1, i__3, &c__2, 2L);
	    i__1 = 2, i__2 = ilaenv(c__2, "CUNMLQ", ch__1, m__, n__, k__, c_n1, 
		    6L, 2L);
	    nbmin = max(i__1,i__2);
	}
    } else {
	...
	cunml2(side, trans, m__, n__, k__, &A(1,1), lda, &TAU(1), &C(1,1)
		, ldc, &WORK(1), &iinfo);
    } else {
	...
	for (i = i1; i3 < 0 ? i >= i2 : i <= i2; i += i3) {
	    ...
	    i__4 = nq - i + 1;
	    clarft("Forward", "Rowwise", i__4, ib, &A(i,i), lda,
		     &TAU(i), t, c__65);
	    if (left) {
	    ...
	    clarfb(side, transt, "Forward", "Rowwise", mi, ni, ib, &A(i,i),
                   lda, t, c__65, &C(ic,jc), ldc, &WORK(1), ldwork);
	}
    }
