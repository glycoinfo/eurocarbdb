...
<#assign  maintainDigestId = digestId-1>
...
<form action="digest_assign.action">
<table width="200">
<tr>Classification Selection</tr>

	<input type="hidden" label="Profile Id" name="profileId" value="${profileId}"/>
	<input type="hidden" label="Digest Id" name="digestId" value="${refineDigestId}"/>
	<input type="hidden" label="Digest Refine List" name="refineDigestId" value="${refineDigestId}"/>
	<input type="hidden" label="refine score" name="criteriaResults" value="${criteriaResults}"/>
...
<h1>Digest Assignments</h1>

<p>test to check digest id: ${digestId} and freemarker version: ${maintainDigestId} and this: ${refineDigestId}</p>

<p>Proceed onto <a href="show_digestionAnalysis.action?digestId=${digestId}&profileId=${profileId}"> digest analysis</a></p>
<p>Alternatively try <a href="refine.action?profileId=${profileId}"> refining the data</a></p>
...
<#if resultsSize ==0 >
<p>Your refinement selection generated no results; the entire result set is displayed below.</p>
</#if>

<#if "refineAssignment"?matches("yes")>
<p>yoyo</p>
</#if>
...
<table class="table_top_header full_width">
	<#if resultsSize == 0>
 	<tr><th>Name</th><th>Structure</th><th>GU</th><th>DB GU</th><th>Area</th><th>Remove</th></tr>
      <#list displayDigest as p>

	<tr><td><a href="show_glycanEntry.action?glycanId=${p[0]}&trueCT=yes&imageStyle=uoxf">${p[4]}</a></td><td align="center"><img src="get_sugar_image.action?download=true&scale=0.4&notation=uoxf&outputType=png&glycanSequenceId=${p[5]?c}" ></td><td>${p[2]}</td><td>${p[3]}</td><td>${p[1]}</td><td><a href="digest_assign.action?profileId=${profileId}&digestId=${digestId}&deleteEntry=${p[6]?c}">delete</td></tr>
      	<#if p_has_next></#if>	
      	</#list></#if>
	...
	<#list showCriteria as show>
	 <tr><td><a href="show_glycanEntry.action?glycanId=${show[4]}&trueCT=yes&imageStyle=uoxf">${show[0]}</a></td><td><img src="get_sugar_image.action?download=true&scale=0.4&notation=uoxf&outputType=png&glycanSequenceId=${show[4]?c}" ></td><td>${show[1]?c}</td><td>${show[2]?c}</td><td>${show[3]}</td><td><a href="digest_assign.action?profileId=${profileId}&digestId=${digestId}&deleteEntry=${show[5]?c}">delete</tr>
	         <#if show_has_next></#if>
		         </#list>
			         </#if>
