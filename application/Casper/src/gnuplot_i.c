...
void gnuplot_set_ylabel(gnuplot_ctrl * h, char * label)
{
    ...
    sprintf(cmd, "set y2label \"%s\"", label) ;
    gnuplot_cmd(h, cmd) ;
    return ;
}
...
)
{
    ...
    if (title == NULL) {
        sprintf(line, "%s \"%s\" axes x1y2 with %s", cmd, name, handle->pstyle) ;
    } else {
        sprintf(line, "%s \"%s\" axes x1y2 title \"%s\" with %s", cmd, name,
                      title, handle->pstyle) ;
    }
...
/*-------------------------------------------------------------------------*/
/**
  @brief	Plot a 2d graph from a list of points using 2nd linestyle.
  @param	handle		Gnuplot session control handle.
  @param	x			Pointer to a list of x coordinates.
  @param	y			Pointer to a list of y coordinates.
  @param	n			Number of doubles in x (assumed the same as in y).
...
)
{
    ...
    if (title == NULL) {
        ...
        sprintf(line, "%s \"%s\" axes x1y2 title \"%s\" with %s 2 2", cmd, name,
                      title, handle->pstyle) ;
    }
